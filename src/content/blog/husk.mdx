...
heroImageAlt: 'husk wordmark on a dark background with badges reading quarantine not delete, confidence scored, and gui plus cli one engine'
category: 'project'
tags:
  - swift
  - swiftui
...
**Matching is a confidence ladder, not a boolean.** A bundle identifier found inside a file, or the executable itself living in the bundle, is `exact`. A team identifier or a folder name matching the app name exactly is `likely` and is pre-checked by default. Anything weaker, fuzzy name resemblance, is `possible`, requires an explicit opt-in, and is never pre-selected. Names under five characters and a denylist of generic words (`mail`, `notes`, `code`, `sync`) are excluded from fuzzy matching entirely, since that's where most uninstallers cause real damage.
